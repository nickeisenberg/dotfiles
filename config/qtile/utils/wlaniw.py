...
def process_essid_and_quality(
    ...
    ethernet_message_format: str,
    use_ethernet: bool,
    format: str,
    ethernet_interface_not_found: bool
):
    try:
        ...
        ipaddr = "N/A"
        if not disconnected:
            ipaddr = get_private_ip(interface)

        else:
            if use_ethernet:
                ipaddr = get_private_ip(ethernet_interface)
                try:
                    with open(f"/sys/class/net/{ethernet_interface}/operstate") as statfile:
                        if statfile.read().strip() == "up":
                            return ethernet_message_format.format(ipaddr=ipaddr)

                        else:
                            return disconnected_message

                except FileNotFoundError:
                    if not ethernet_interface_not_found:
                        logger.error("Ethernet interface has not been found!")
                        ethernet_interface_not_found = True
                    return disconnected_message
...
class WlanIw(GenPollCommand):
    ...
    def parse(self, raw: str):
        ...
        return process_essid_and_quality(
            ...
            disconnected_message=self.disconnected_message,
            ethernet_message_format=self.ethernet_message_format,
            use_ethernet=self.use_ethernet,
            format=self.format,
            ethernet_interface_not_found=self.ethernet_interface_not_found,
        )
